...
> 历史审查记录：以下测试结论和设备状态仅对应记录日期。当前使用说明见[渲染组件](rendering-components.md)，不以历史安装状态代表最新版本。

## 范围与已修复问题

- 用户补充测试截图后确认，上一轮修复还不完整：图片和点击区域已经生成，但所有徽章仍呈灰块。手机常用 QA 当时仍为 `2.4.8-revised.7-20260910-010743`，不包含后续修复。用户原始截图不随仓库公开。
