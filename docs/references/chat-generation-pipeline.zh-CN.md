...
随后，`GenerationHandler` 加入已启用的知识库工具。前台聊天模型不再挂载记忆提取
工具；提取改为回答完成后，根据当前对话的有效记忆设置独立调度。
...
记忆 RAG 使用相似的查询链路。事实记忆沿用助手专属或全局范围，情景记忆和对话片段
按对话隔离。检索合并兼容向量与词法命中，在各自文本预算内选取最多六条有效记忆和
四条片段，并排除已在待发送上下文中的片段。记忆被标记为信息而非应执行的指令。

新对话默认白板；分支继承来源对话的记忆模式，后续索引归新对话所有。用户可在
「更多 → 最近记忆」选择跟随助手、启用、关闭、仅 RAG 或仅记忆提取模型。仅提取模式
将已保存的有效记忆作为基础上下文，不调用向量模型。嵌入模型仅建立片段索引，
不会生成事实或情景摘要。

回答完成后，`ChatService` 独立调度 `MemoryExtractionService`，通过批次、指纹、
检查点和冷却避免每次重生成都重复提取；空结果不创建记录，失败可查看并手动重试。
写入 Room 前重新检查对话策略与任务身份。情景记忆在 Active、Completed、Superseded
间更迭，日常检索只使用有效状态。稳定记忆 ID、版本 ID、内容哈希和嵌入键分别标识
记录、版本和索引，详见[记忆模式](../memory-extraction-only.md)和
[记忆身份](../memory-identity-and-readability.md)。
