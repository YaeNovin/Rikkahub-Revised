...
fun UpdateCard(vm: ChatVM) {
    ...
    state.onSuccess { info ->
        ...
        val latest = remember(info.version) { Version(info.version) }
        if (latest > current && !dismissed) {
            Card(onClick = { showDetail = true }) {
                Column(
                    modifier = Modifier
                        .padding(8.dp)
                        .fillMaxWidth(),
                    ...
                    Row(
                        ...
                        Text(
                            text = stringResource(
                                R.string.update_card_new_version_found,
                                info.version,
                            ),
                            style = MaterialTheme.typography.titleMedium,
        ...
        if (showDetail) {
            ...
            ModalBottomSheet(
                ...
                Column(
                    ...
                    runCatching {
                        Instant.parse(info.publishedAt).toJavaInstant().toLocalDateTime()
                    }.getOrNull()?.let { publishedAt ->
                        Text(
                            text = publishedAt,
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.primary,
                    ...
                    if (info.downloads.isEmpty()) {
                        Text(
                            text = stringResource(R.string.update_card_no_compatible_download),
                            style = MaterialTheme.typography.bodyMedium,
                            color = MaterialTheme.colorScheme.error,
                        )
                    }
