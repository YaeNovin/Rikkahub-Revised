                ...
                item(
                    ...
                    supportingContent = {
                        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                            ...
                            S3Config.selectableBackupItems.forEach { item ->
                                Row(
                                    modifier = Modifier.fillMaxWidth(),
                                    ...
                                    Switch(
                                        ...
                                        onCheckedChange = { checked ->
                                            updateS3Config(
                                                s3Config.copy(
                                                    items = s3Config.items.withS3Scope(item, checked),
                                                )
                                            )
                                        },
                                    )
...
private fun List<S3Config.BackupItem>.withS3Scope(
    item: S3Config.BackupItem,
    checked: Boolean,
): List<S3Config.BackupItem> = when {
    item == S3Config.BackupItem.WORKSPACE && checked ->
        (this + S3Config.BackupItem.DATABASE + item)
            .filterNot { it == S3Config.BackupItem.FILES }
            .distinct()

    item == S3Config.BackupItem.DATABASE && !checked ->
        filterNot { it == S3Config.BackupItem.DATABASE || it == S3Config.BackupItem.WORKSPACE }

    item == S3Config.BackupItem.ATTACHMENTS ->
        (filterNot { it == S3Config.BackupItem.ATTACHMENTS || it == S3Config.BackupItem.FILES } +
...
@Composable
private fun S3Config.BackupItem.toBackupScopeLabel(): String = when (this) {
    S3Config.BackupItem.DATABASE -> stringResource(R.string.backup_page_database)
    S3Config.BackupItem.TOKENS -> stringResource(R.string.backup_page_tokens)
    S3Config.BackupItem.ATTACHMENTS -> stringResource(R.string.backup_page_attachments)
    S3Config.BackupItem.WORKSPACE -> stringResource(R.string.backup_page_workspace)
    S3Config.BackupItem.FILES -> error("Legacy backup item must not be displayed")
}
