...
fun assumeLatexSize(latex: String, fontSize: Float): Rect? {
    return latexRenderCandidates(latex).firstNotNullOfOrNull { candidate ->
        runCatching {
            JLatexMathDrawable.builder(candidate)
                .textSize(fontSize)
                .padding(0)
                .build()
                .bounds
...
fun splitLatex(
    ...
    return latexRenderCandidates(latex).firstNotNullOfOrNull { candidate ->
        runCatching {
            JLatexMathSplitter.split(candidate, maxWidthPx, fontSize, color)
