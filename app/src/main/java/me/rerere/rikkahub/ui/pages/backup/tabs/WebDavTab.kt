            ...
            CardGroup {
                item(
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        Text(
                            text = stringResource(R.string.backup_page_backup_items_desc),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                        WebDavConfig.selectableBackupItems.forEach { item ->
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                verticalAlignment = Alignment.CenterVertically,
                            ) {
                                Text(
                                    text = item.toBackupScopeLabel(),
                                    modifier = Modifier.weight(1f),
                                    style = MaterialTheme.typography.bodyMedium,
                                )
                                Switch(
                                    checked = BackupPolicy.hasScope(
                                        webDavConfig.items,
                                        BackupScope.valueOf(item.name),
                                    ),
                                    onCheckedChange = { checked ->
                                        updateWebDavConfig(
                                            webDavConfig.copy(
                                                items = webDavConfig.items.withWebDavScope(item, checked),
                                            )
                                        )
                                    },
                                )
                        ...
                        Text(
                            text = stringResource(R.string.backup_page_workspace_requires_database),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                        Text(
                            text = stringResource(R.string.backup_page_rootfs_excluded),
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                    }
...
private fun List<WebDavConfig.BackupItem>.withWebDavScope(
    item: WebDavConfig.BackupItem,
    checked: Boolean,
): List<WebDavConfig.BackupItem> = when {
    item == WebDavConfig.BackupItem.WORKSPACE && checked ->
        (this + WebDavConfig.BackupItem.DATABASE + item)
            .filterNot { it == WebDavConfig.BackupItem.FILES }
            .distinct()

    item == WebDavConfig.BackupItem.DATABASE && !checked ->
        filterNot { it == WebDavConfig.BackupItem.DATABASE || it == WebDavConfig.BackupItem.WORKSPACE }

    item == WebDavConfig.BackupItem.ATTACHMENTS ->
        (filterNot { it == WebDavConfig.BackupItem.ATTACHMENTS || it == WebDavConfig.BackupItem.FILES } +
            if (checked) listOf(item) else emptyList()).distinct()

    checked -> (this + item).distinct()
    else -> this - item
}

@Composable
private fun WebDavConfig.BackupItem.toBackupScopeLabel(): String = when (this) {
    WebDavConfig.BackupItem.DATABASE -> stringResource(R.string.backup_page_database)
    WebDavConfig.BackupItem.TOKENS -> stringResource(R.string.backup_page_tokens)
    WebDavConfig.BackupItem.ATTACHMENTS -> stringResource(R.string.backup_page_attachments)
    WebDavConfig.BackupItem.WORKSPACE -> stringResource(R.string.backup_page_workspace)
    WebDavConfig.BackupItem.FILES -> error("Legacy backup item must not be displayed")
}
