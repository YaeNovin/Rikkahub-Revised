...
class S3Sync(
    ...
    suspend fun prepareBackupFile(config: S3Config): File = withContext(Dispatchers.IO) {
        require(BackupPolicy.isValidSelection(config.items)) {
            "Workspace backups require a database backup"
        }
        ...
        ZipOutputStream(FileOutputStream(backupFile)).use { zipOut ->
            if (BackupPolicy.hasScope(config.items, BackupScope.TOKENS)) {
                addVirtualFileToZip(
                    zipOut = zipOut,
                    name = BackupPolicy.SETTINGS_ENTRY,
            ...
            if (BackupPolicy.workspaceBackupAllowed(config.items)) {
                val workspacesFolder = File(context.filesDir, BackupPolicy.WORKSPACES_DIRECTORY)
                workspacesFolder.listFiles()
                    ?.filter { it.isDirectory }
                    ?.forEach { workspaceFolder ->
                        val workspaceFiles = File(workspaceFolder, "files")
                        if (workspaceFiles.isDirectory) {
                            addDirectoryToZip(
                                zipOut = zipOut,
                                rootDir = workspaceFiles,
                                currentDir = workspaceFiles,
                    ...
                    when (zipEntry.name) {
                        BackupPolicy.SETTINGS_ENTRY -> {
                            if (BackupPolicy.hasScope(config.items, BackupScope.TOKENS)) {
                                ...
                                Log.i(TAG, "restoreFromBackupFile: Skipping credentials and settings")
                            }
                        }
                        ...
                        "rikka_hub.db", "rikka_hub-wal", "rikka_hub-shm" -> {
                            ...
                            if (BackupPolicy.hasScope(config.items, BackupScope.ATTACHMENTS) &&
                                ...
                                val fileName = BackupPolicy.safeRelativePath(zipEntry.name, FileFolders.UPLOAD)
                                    ?: throw Exception("Invalid attachment path in backup: ${zipEntry.name}")
                                ...
                                val fileName = BackupPolicy.safeRelativePath(zipEntry.name, FileFolders.FONTS)
                                    ?: throw Exception("Invalid font path in backup: ${zipEntry.name}")
                            ...
                            } else if (BackupPolicy.workspaceBackupAllowed(config.items) &&
                                zipEntry.name.startsWith("${BackupPolicy.WORKSPACES_DIRECTORY}/")
                            ) {
                                val relativePath = BackupPolicy.workspaceRestoreRelativePath(zipEntry.name)
                                    ?: throw Exception("Invalid workspace path in backup: ${zipEntry.name}")
                                val workspacesFolder = File(context.filesDir, BackupPolicy.WORKSPACES_DIRECTORY)
